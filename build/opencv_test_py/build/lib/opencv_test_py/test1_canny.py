...
class ImageSubscriber(Node):
  ...
  def listener_callback(self, data):
    ...
    circles = cv2.HoughCircles(current_frame_gray, 
                            cv2.HOUGH_GRADIENT, 
                            dp=1,   # ratio of accumulator circle array and input image resolution
                            minDist=20,   # minimum distance between 2 detected circle to avoid detecting multiple circles for same object
                            param1=100,   # upper threshold for canny edge 
                            param2=30,    # threshold to filted detected centers
                            minRadius=0,   # min and max radium of circles to be detected
                            maxRadius=0)
    

    # we define the message datatype to initializew
    ...
    # corners = cv2.dilate(corners,None)0
    ...
    np.save('circle_centers.npy', np.array(detected_centers))
